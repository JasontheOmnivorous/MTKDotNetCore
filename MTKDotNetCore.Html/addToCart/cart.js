...
function createProduct(name, description, price) {
  ...
  const requestProduct = {
    productId: lst.length > 0 ? lst[lst.length - 1].id + 1 : 1,
    productName: name,
...
function addToCartProduct(id) {
  ...
  let productLst = lst.filter((x) => x.productId === parseInt(id));
  let product = productLst[0];

  let index = cartLst.findIndex((x) => x.cartProductId === parseInt(id));
...
function increaseCart(id) {
  let cartLst = getCart();
  let index = cartLst.findIndex((x) => x.cartId === parseInt(id));
...
function decreaseCart(id) {
  ...
  if (index !== null) {
    if (cartLst[index].cartQuantity === 1) {
      deleteCart(cartLst[index].cartId);
    } else {
      cartLst[index].cartQuantity -= 1;
      const cartStr = JSON.stringify(cartLst);
      localStorage.setItem(tblCart, cartStr);
